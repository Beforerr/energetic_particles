...
begin
    ...
    Δs_perp_weighted_mean = mean(df.Δs_perp, w) ± std(df.Δs_perp, w)
    Δs_para_weighted_mean = mean(df.Δs_para, w) ± std(df.Δs_para, w)

    # Add horizontal lines for weighted means
    hlines_measurement!(fg.grid[1, 1].axis, Δs_para_weighted_mean; label="<Δs_∥>: $(Δs_para_weighted_mean)", color=:blue)
    hlines_measurement!(fg.grid[2, 1].axis, Δs_perp_weighted_mean; label="<Δs_⊥>: $(Δs_perp_weighted_mean)", color=:blue)
...
avg_df = @chain result begin
    ...
    @groupby(:β, :v)
    @combine begin
        :Δs_perp = mean(:Δs_perp, StatsBase.weights(:w)) ± std(:Δs_perp, StatsBase.weights(:w))
        :Δs_para = mean(:Δs_para, StatsBase.weights(:w)) ± std(:Δs_para, StatsBase.weights(:w))
    end
    sort!(:β)
end
spec = data(avg_df) * errorbar_visual
spec *= mapping(:β, [m.Δs_para, m.Δs_perp], row=dims(1) => renamer(["Parallel", "Perpendicular"]))
spec *= (; color=:v => nonnumeric, dodge_x=:v => nonnumeric)
fg = draw(spec, scales(DodgeX=(; width=2)))
easy_save("Δs_βscan_vscan_weighted")
```
